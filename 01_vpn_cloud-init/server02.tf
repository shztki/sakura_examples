resource "sakuracloud_server" "server02" {
  ...
  user_data = templatefile(var.server02["cloud_init_file"], {
    ...
    if1_name            = "ens3"            # [eth0,ens3]
    connection1_name    = "cloud-init ens3" # [System,cloud-init] [eth0,ens3]
    if2_name            = "ens4"            # [eth1,ens4]
    connection2_name    = "cloud-init ens4" # [System,cloud-init] [eth1,ens4]
    ## private
    ...
    ## router+switch
    #nic1ip         = sakuracloud_internet.router01[0].ip_addresses[var.server01["count"] + count.index],
    ...
    nic2ip     = cidrhost(var.switch02["name"], var.server02["start_ip"] + count.index),
  ...
  #      format("hostnamectl set-hostname %s-%s%03d", module.label.id, var.server02["name"], count.index + 1),
  ...
  #        cidrhost(var.switch01["name"], var.server02["start_ip"] + count.index),
  ...
  #      #  cidrhost(var.switch02["name"], var.server02["start_ip"] + count.index),
