resource "sakuracloud_server" "server04" {
  ...
  user_data = templatefile(var.server04["cloud_init_file"], {
    ssh_authorized_keys = sakuracloud_ssh_key.sshkey01.public_key,
    password            = var.default_password_hash,
    fqdn                = format("%s-%s-%03d", module.label.id, var.server04["name"], count.index + 1),
    ## private
    nic1ip         = cidrhost(var.switch01["name"], var.server04["start_ip"] + count.index),
    nic1cidr       = element(regex("^\\d+\\.\\d+\\.\\d+\\.\\d+/(\\d+)", var.switch01["name"]), 0),
    defaultgateway = cidrhost(var.switch01["name"], var.vpn_router01["vip"]),
    ## router+switch
    #nic1ip         = sakuracloud_internet.router01[0].ip_addresses[var.server01["count"] + var.server02["count"] + var.server03["count"] + count.index],
    ...
    nic2ip     = cidrhost(var.switch02["name"], var.server04["start_ip"] + count.index),
  ...
  #    fqdn              = format("%s-%s%03d.local", module.label.id, var.server04["name"], count.index + 1)
  ...
  #        cidrhost(var.switch01["name"], var.server04["start_ip"] + count.index),
  ...
  #      #  cidrhost(var.switch02["name"], var.server04["start_ip"] + count.index),
  #      #  element(regex("^\\d+\\.\\d+\\.\\d+\\.\\d+/(\\d+)", var.switch02["name"]), 0)
  #      #),
  #      ## --- 92: ens4 (private) ※GW/DNSなし（静的ルートのみ付与） ---
  #      #format("cat > /etc/netplan/92-ens4.yaml <<'NETCFG'\nnetwork:\n  version: 2\n  ethernets:\n    ens4:\n      dhcp4: false\n      dhcp6: false\n      addresses: [\"%s/%s\"]\n      routes:\n        - to: %s/%s\n          via: %s\nNETCFG",
  #      #  cidrhost(var.switch02["name"], var.server04["start_ip"] + count.index),                            # ens4のアドレス
