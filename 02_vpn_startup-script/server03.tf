resource "sakuracloud_server" "server03" {
  ...
  disk_edit_parameter {
    ip_address = cidrhost(var.switch01["name"], var.server03["start_ip"] + count.index)
    gateway    = cidrhost(var.switch01["name"], var.vpn_router01["vip"])
    netmask    = tostring(element(regex("^\\d+.\\d+.\\d+.\\d+/(\\d+)", var.switch01["name"]), 0))
    #ip_address      = sakuracloud_internet.router01[0].ip_addresses[3 + var.server01["count"] + var.server02["count"] + count.index]
    ...
    note {
      id = sakuracloud_note.init_note02.id
      variables = {
        #user_name   = "sakura"
