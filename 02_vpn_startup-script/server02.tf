resource "sakuracloud_server" "server02" {
  ...
  disk_edit_parameter {
    ...
    #ip_address      = sakuracloud_internet.router01[0].ip_addresses[3 + var.server01["count"] + count.index]
    ...
    note {
      id = sakuracloud_note.init_note01.id
      variables = {
        #user_name   = "sakura"
        #eth1_ip     = format("%s/%s", cidrhost(var.switch02["name"], var.server02["start_ip"] + count.index), element(regex("^\\d+.\\d+.\\d+.\\d+/(\\d+)", var.switch02["name"]), 0))
